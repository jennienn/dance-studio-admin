-- 개인레슨 예약 시작 시간을 10:00~22:00(30분 단위)로 확장한다.
alter table solo_bookings drop constraint if exists solo_bookings_start_minute_check;
alter table solo_bookings add constraint solo_bookings_start_minute_check
  check (start_minute between 600 and 1320 and start_minute % 30 = 0);
...
begin
  ...
  if (
    select count(*) from solo_bookings
    where cycle_id = p_cycle_id and booking_date = p_date and status in ('confirmed', 'completed')
  ) >= 2 then
